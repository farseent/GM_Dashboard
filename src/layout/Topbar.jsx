  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-border-subtle bg-surface-raised px-4 sm:px-8">
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="rounded-md p-1.5 text-brand-500 hover:bg-surface lg:hidden"
          aria-label="Open menu"
        >
          <Menu className="h-5 w-5" />
        </button>
        <h1 className="text-base font-semibold text-brand-950 sm:text-lg">{title}</h1>
      </div>

      <div className="flex items-center gap-2 sm:gap-4">
        <button className="relative rounded-full p-2 text-brand-500 hover:bg-surface">
        ...
        <button className="flex items-center gap-2 rounded-lg border border-border-subtle px-2 py-1.5 hover:bg-surface sm:px-3">
          ...
          <span className="hidden text-sm font-medium text-brand-950 sm:inline">General Manager</span>
          <ChevronDown className="hidden h-4 w-4 text-brand-500 sm:inline" />
        </button>
      </div>
    </header>
  )
